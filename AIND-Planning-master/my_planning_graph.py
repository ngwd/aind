...
class PgNode():
    ...
    def type_s(self):
        return True if type(self).__name__=="PgNode_s" else False

    def type_a(self):
        return True if type(self).__name__=="PgNode_a" else False
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        state_pos = [ v.symbol for v in self.s_levels[level] if v.is_pos==True ]
        state_neg = [ v.symbol for v in self.s_levels[level] if v.is_pos==False]

        for act in self.all_actions:
            to_be_added = all(v in state_pos for v in act.precond_pos)
            if to_be_added == False:
                continue
            to_be_added = all(v in state_neg for v in act.precond_neg)
            if to_be_added == True:
                pa = PgNode_a(act)
                for v in pa.prenodes:
                    pa.parents.add(v)
                self.a_levels[level].add(pa)
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set())  # S0 set of s_nodes - empty to start
        for v in self.a_levels[level-1]:
            for u in v.effnodes:
                self.s_levels[level].add(u)
                v.children.add(u)
                u.parents.add(v)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if collections.Counter(node_a1.action.precond_pos)== \
           collections.Counter(node_a2.action.precond_pos) and \
           collections.Counter(node_a1.action.precond_neg)== \
           collections.Counter(node_a2.action.precond_neg):
            return collections.Counter(node_a1.action.effect_add)== \
                   collections.Counter(node_a2.action.effect_rem) and \
                   collections.Counter(node_a1.action.effect_rem)== \
                   collections.Counter(node_a2.action.effect_add) 
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return (collections.Counter(node_a1.action.effect_rem)== \
                collections.Counter(node_a2.action.precond_pos) and \
                collections.Counter(node_a1.action.effect_add)== \
                collections.Counter(node_a2.action.precond_neg)) or \
               (collections.Counter(node_a2.action.effect_rem)== \
                collections.Counter(node_a1.action.precond_pos) and \
                collections.Counter(node_a2.action.effect_add)== \
                collections.Counter(node_a1.action.precond_neg))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return ((len(node_a1.action.precond_pos) or \
                len(node_a2.action.precond_pos) or \
                len(node_a1.action.precond_neg) or \
                len(node_a2.action.precond_neg)) and \
               (collections.Counter(node_a1.action.precond_pos)== \
                collections.Counter(node_a2.action.precond_neg) and \
                collections.Counter(node_a1.action.precond_neg)== \
                collections.Counter(node_a2.action.precond_pos))) or\
                all(v.is_mutex(u) for v in node_a1.parents for u in node_a2.parents)
        """
        temp = True
        for v in node_a1.parents:
            for u in node_a2.parents:
                temp = temp and v.is_mutex(u)
        """
        return result
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return (node_s1.__class__ is PgNode_s and \
                isinstance(node_s2, node_s1.__class__) and \
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a1 in node_s1.parents:
            for a2 in node_s2.parents:
                if (a1.is_mutex(a2) or a2.is_mutex(a1)) ==False or \
                    node_s2 in a1.children or node_s1 in a2.children:
                    return False
        return True 
