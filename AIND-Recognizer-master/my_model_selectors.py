...
class SelectorBIC(ModelSelector):
    ...
    logL is the score of model, log likelyhood
    p is the number of parameters
    N is the number of data points = length of X
    """
    ...
    def select(self):
        ...
        for num_states in range(self.min_n_components, self.max_n_components): 
            ...
            p = num_states + num_states*(num_states-1)  + 2*num_states * num_features
            ...
            try:
                ...
                for w in self.hwords:
                    if w!=self.this_word:
                        ...
                        anti_logL += model.score(X, lengths)
            except:
                print("failure on {} with {} states".format(self.this_word, num_states))
                continue
            anti_logL /= float(len(self.hwords)-1)  # normalize
            DIC = logL - anti_logL
            if DIC > max_DIC:
                max_DIC = DIC
                best_model = model
        if max_DIC == float("-inf"):
            return None
        return best_model
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        best_model = None 
        for num_states in range(self.min_n_components, self.max_n_components): 
            if num_states > len(self.lengths): 
                break
            # print ("state count {}".format(num_states))
            ...
            for cv_train_idx, cv_test_idx in split_method.split(self.sequences):
                ...
                try:
                    ...
                    cv_test_logL  = model.score(cv_test_X,  cv_test_lengths)
                except:
                    print("failure on {} with {} states".format(self.this_word, num_states))
                    continue
                if cv_test_logL > max_logL:
                    max_logL = cv_test_logL 
                    best_model = model 
        return best_model
